...
class PosePlayer(hm.HelloNode):
    ...
    def _play_cb(self, request, response):
        try:
            #  ← reload on every request so you get the latest joint values
            self.poses = json.loads(POSE_FILE.read_text())
...
def main():
    node = PosePlayer()
    node.run()                                     # ← makes it a full node

    exec_ = MultiThreadedExecutor(num_threads=2)   # extra worker thread
    exec_.add_node(node)

    try:
        exec_.spin()                               # runs until Ctrl-C
